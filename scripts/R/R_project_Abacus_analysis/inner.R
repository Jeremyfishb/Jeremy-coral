Mcapdata <- read.csv("Abacus_output_inner.csv",header = T,row.names = 1)
...
Mcap.2peps<-subset(nsaf.uniqpeps, select=X2018_MARCH_12_CORAL_JAA_23_ADJNSAF:X2018_MARCH_22_JPG_CORAL_19_ADJNSAF, nsaf.uniqpeps[,21]>1) #filters out 1 count peps
...
Mcap.spc2peps<-subset(spc.uniqpeps, select=X2018_MARCH_12_CORAL_JAA_23_NUMSPECSTOT:X2018_MARCH_22_JPG_CORAL_19_NUMSPECSTOT, spc.uniqpeps[,21]>1)
prot.len<-subset(Mcapdata, select=PROTLEN)
Mcap.qspec<-merge(x=Mcap.spc2peps, y=prot.len, by='row.names', all.x=T)
write.csv(Mcap.qspec, 'Mcap_inner_spec_counts_for_qspec.csv')
...
ordiplot(Mcap.nmds, choices=c(1,2), type='text', display='sites', cex=0.5, xlim = c(-.2,.2),
         ylim= c(-.1,.1), main = "inner samples") # end here makes rough plot- look for outliers(and remove),(normally would check replicates here)
...
ordiplot(Mcap.nmds, choices = c(1, 2), type="text", display = "sites",
         xlab="Axis 1", ylab="Axis 2", xlim = c(-0.05, 0.05), ylim = c(-0.05, 0.05),
         main = "inner samples") #plot again
...
write.csv(sig.prots001, file = "inner_samples_sig_prots001") # save as text csv

sig.prots01 <- subset(Pvals,Pvals < 0.01) #subset pvals < 0.01
length(sig.prots01)
write.csv(sig.prots01, file = "inner_samples_sig_prots01")
