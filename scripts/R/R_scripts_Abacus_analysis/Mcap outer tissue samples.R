Mcapdata <- read.csv("Abacus_output_Mcap_outer.csv",header = T,row.names = 1)
Mcapdata
...
Mcap.2peps<-subset(nsaf.uniqpeps, select=X2018_MARCH_12_CORAL_JAA_01_ADJNSAF:X2018_MARCH_12_CORAL_JAA_22_ADJNSAF, nsaf.uniqpeps[,23]>1) #filters out 1 count peps
...
ordiplot(Mcap.nmds, choices=c(1,2), type='text', display='sites', cex=0.5, main = "Mcap outer tissue samples") # end here makes rough plot- look for outliers(and remove),(normally would check replicates here)
?ordiplot
