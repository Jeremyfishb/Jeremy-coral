Mcapdata <- read.csv("Abacus_output_Mcap_Rerun_inner_samples.csv",header = T,row.names = 1)
...
Mcap.2peps<-subset(nsaf.uniqpeps, select=X2018_MARCH_12_CORAL_JAA_23_ADJNSAF:X2018_MARCH_22_JPG_CORAL_19_ADJNSAF, nsaf.uniqpeps[,22]>1) #filters out 1 count peps
...
ordiplot(Mcap.nmds, choices=c(1,2), type='text', display='sites', cex=0.5, xlim = c(-.2,.2),
         ylim= c(-.1,.1), main = "Mcap Rerun inner samples") # end here makes rough plot- look for outliers(and remove),(normally would check replicates here)
?ordiplot
